...
document.addEventListener("DOMContentLoaded", function () {
    ...
    //Attempts to retrieve companyAPI from local storage
    function retrieveCompanies() {
        return JSON.parse(localStorage.getItem('companyListInternal')) || [];
    }

    //If companyAPI doesn't exist in local storage it is fetched
    if (companies.length === 0) {
        ...
        async function setComp() {
            const getComps = await fetch(companyAPI);
            const data = await getComps.json();
            localStorage.setItem('companyListInternal', JSON.stringify(data))
            getComp();
        }

        async function getComp() {
            const comp = await JSON.parse(localStorage.getItem('companyListInternal'));
    ...
    //Displays Credits and then hides them after 5 seconds
    ...
    async function popList(companies) {
        ...
        //Matches the inputted value with the company list symbols
        ...
        function infoFill(comp) {
            ...
            document.querySelector("#stockWebsite").style.color = "pink";
    ...
    async function stockData(comp) {
        ...
        if (stocks.length <= 1) {
            ...
            //Sorts the list by the selected header
    ...
    //Changes the displayed divs to be formatted for the second page
    function changeView() {
    ...
    //Changes the displayed divs to be formatted for the first page
    ...
    function makeBar(data) {
        ...
        const financeTable = document.querySelector("box g");
    ...
    async function makeCandle(data) {
        ...
        const boxj = document.querySelector("boxJ");

        if (stocks.length <= 1) {
            console.log("Error: Candlestick Chart Cannot be Created, Stock Data Does not Exist for this Company");
        } else {
